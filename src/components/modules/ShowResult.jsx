...
const ShowResult = () => {
  const { result } = useResultHandler();

  return (
    <div
      className={`fixed w-screen h-screen top-0 left-0 backdrop-blur-sm z-50 transition-all duration-300 ${
        result == null ? "invisible opacity-0" : "visible opacity-100"
      }`}
    >
      <div
        className={`w-full h-full flex items-center justify-center ${
          result ? "bg-green-700/30" : "bg-rose-700/30"
        }`}
      >
        <div className="text-white">
          ...
          <p className="mt-2 text-xl font-bold">
            {result ? "شما برنده شدید" : "متاسفانه شما باختید"}!!!
          </p>
          <StartBtn />
        </div>
      </div>
